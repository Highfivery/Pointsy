...
- ba8d22b: feat(points): put back a completed chore from the activity feed (#145)
...
- 95fff31: Rebrand the homepage and onboarding screens for launch. The homepage is now a
  paced, sectioned landing page — hero with the new logo lockup, how-it-works, an
  app preview, a "Free & open" community block, and an FAQ (which also powers an
  AEO FAQPage in structured data) — leaning into free-forever and open-source. The
  sign-in, create-family, join, and profile-picker screens all get the Pointsy
  logo lockup and a consistent, centered look.

## 0.41.0

### Minor Changes

- 9a53089: Add a PWA install helper so people can add Pointsy to their home screen easily.
...
- 51384fa: Launch SEO/AEO and the pointsy.kids domain. Set the canonical site to
  https://pointsy.kids with OpenGraph/Twitter metadata and a branded dynamic OG
  image, add robots.txt and a sitemap, JSON-LD (WebApplication) on the homepage,
  an llms.txt for AI crawlers, and per-page descriptions for the public pages.
  Also refresh the Privacy Policy and Terms to reflect the new in-app self-serve
...
- 6d33041: Replace the app icon with a "P" monogram on the emerald→cyan brand gradient. The
  previous four-pointed spark looked like another product's logo; the new mark is
  distinctive and reads cleanly from favicon to home-screen size.
